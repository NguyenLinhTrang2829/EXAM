        ...
        where TKey : IEquatable<TKey>
    {
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = user.Id, code }, httpsContext.Request.Scheme);
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null, bool IsCalledFromRegisterWithoutUsername = false)
        {
            ...
            if (result.Succeeded)
            {
                ...
                code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code }, httpsContext.Request.Scheme);
        ...
        private async Task<LoggedOutViewModel> BuildLoggedOutViewModelAsync(string logoutId)
        {
            ...
            if (User?.Identity.IsAuthenticated == true)
            {
                ...
                {
                    var providerSupportsSignout = await httpsContext.GetSchemeSupportsSignOutAsync(idp);
